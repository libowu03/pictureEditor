...
#include "ncnn_faceboxesv2.h"
#include "../utils.h"

using ncnncv::NCNNFaceBoxesV2;

NCNNFaceBoxesV2::NCNNFaceBoxesV2(const std::string &_param_path,
...
void NCNNFaceBoxesV2::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  cv::Mat mat_rs;
...
void NCNNFaceBoxesV2::detect(const cv::Mat &mat, std::vector<types::Boxf> &detected_boxes,
                                      ...
                                      float img_height, float img_width)
{
  ...
  extractor.extract("loc", bboxes); // c=1 h=? w=4
  extractor.extract("conf", probs); // c=1 h=? w=2
#ifdef LITENCNN_DEBUG
  BasicNCNNHandler::print_shape(bboxes, "loc");
...
void NCNNFaceBoxesV2::nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
