...
LUT3DContext *cubeFileToPng(char *filename);
...
#define DEFINE_INTERP_FUNC(name, nbits)                                                                                                   \
    static int interp_##nbits##_##name(LUT3DContext *lut3d, const uint8_t *indata, uint8_t *outdata, int width, int height, int linesize,float strength) \
    {                                                                                                                                     \
        int x, y;                                                                                                                         \
        ...
        {                                                                                                                                 \
            ...
            {                                                                                                                             \
                const struct rgbvec scaled_rgb = {src[x + r] * scale,                                                                     \
                                                  ...
                                                  src[x + b] * scale};                                                                    \
                struct rgbvec vec = interp_##name(lut3d, &scaled_rgb);                                                                    \
                dst[x + r] = clip_uint##nbits(src[x + r] - (src[x + r] - clip_uint##nbits(vec.r * (float)((1 << nbits) - 1))) * strength);     \
                dst[x + g] = clip_uint##nbits(src[x + g] - (src[x + g] - clip_uint##nbits(vec.g * (float)((1 << nbits) - 1))) * strength);     \
                dst[x + b] = clip_uint##nbits(src[x + b] - (src[x + b] - clip_uint##nbits(vec.b * (float)((1 << nbits) - 1))) * strength);     \
                if (!direct && step == 4)                                                                                                 \
                    dst[x + a] = src[x + a];                                                                                              \
            }                                                                                                                             \
            dstrow += linesize;                                                                                                           \
...
#define SET_PLANAR_FUNC(name)                         \
    ...
    {                                                 \
        ...
        {                                             \
            ...
            {                                         \
            case 8:                                   \
                interp_func = interp_8_##name##_p8;   \
                break;                                \
            case 9:                                   \
                interp_func = interp_16_##name##_p9;  \
                break;                                \
            case 10:                                  \
                interp_func = interp_16_##name##_p10; \
                break;                                \
            case 12:                                  \
                interp_func = interp_16_##name##_p12; \
                break;                                \
            case 14:                                  \
                interp_func = interp_16_##name##_p14; \
                break;                                \
            case 16:                                  \
                interp_func = interp_16_##name##_p16; \
                break;                                \
            }                                         \
        }                                             \
    } while (0)
...
#define SET_FUNC(name)                      \
    ...
    {                                       \
        if (is16bit)                        \
        {                                   \
            interp_func = interp_16_##name; \
        }                                   \
        else                                \
        {                                   \
            interp_func = interp_8_##name;  \
        }                                   \
    } while (0)
    ...
    interp_func(lut3d, indata, outdata, width, height, linesize,strength);
    free(lut3d);
    return (0);
}
 ...
 * @return
 */
jobject changeLutPyPng(JNIEnv *env, jclass clazz, jobject bitmap,jobject lutMap){
    ...
    f_LUTFilter(img.data,img.cols,img.rows,img.step,mapLut.data);
    cvtColor(img,img,COLOR_BGRA2RGBA);
    jobject resultBitmap = matToBitmap(env, img, true, bitmap);
...
jobject changeByLut(JNIEnv *env, jclass clazz, jobject bitmap, char *lutfile,jfloat strength){
    try{
        AndroidBitmapInfo info;
        void *pixels;
        CV_Assert(AndroidBitmap_getInfo(env, bitmap, &info) >= 0);
        ...
        int Height = 0;
        int Channels = 0;
        unsigned char *inputImage = img.ptr<uchar>();
        Width = img.cols;
        Height = img.rows;
        ...
        if ((Channels != 0) && (Width != 0) && (Height != 0))
        {
            ...
            apply_lut(lutfile, inputImage, outputImg, Width, Height, Width * Channels, Channels, interp_mode,is16bit,strength);
            Mat result(img.size(), img.type(), outputImg);
...
jobject cubeToPng(JNIEnv *env, jclass clazz, char *lutfile,jobject bitmap){
    Mat result(512,512,CV_8UC4,Scalar(255,255,255,255));
    LUT3DContext  *t = cubeFileToPng(lutfile);
    for (int ir = 0; ir < 64; ir++) {
        for (int ig = 0; ig < 64; ig++) {
            for (int ib = 0; ib < 64; ib++) {
                int x = ir + (ib % 8) * 64;
                int y = (ig + floor(ib / 8) * 64);
                struct rgbvec c;
                c.r = ir/64.0*(t->lutsize);
                c.g = ig/64.0*(t->lutsize);
                c.b = ib/64.0*(t->lutsize);
                rgbvec re = interp_trilinear(t,&c);
                result.data[4 * (x + 512 * y) + 0] = (int)round(255.0 * re.b);
                result.data[4 * (x + 512 * y) + 1] = (int)round(255.0 * re.g);
                result.data[4 * (x + 512 * y) + 2] = (int)round(255.0 * re.r);
                result.data[4 * (x + 512 * y) + 3] = 255;
            }
        }
    }
    cv::cvtColor(result,result,CV_BGRA2RGBA);
    jobject resultBitmap = matToBitmap(env, result, true, bitmap);
    result.release();
    AndroidBitmap_unlockPixels(env, bitmap);
    return resultBitmap;
